...
def upload():
    ...
    c.execute("""
        ...
        (username, filename, encrypted_key, iv,
         service_option, photo_size, note, payment_method, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0)
    """, (
        username, filename, encrypted_key, iv,
...
def admin_dashboard():
    ...
    c.execute("SELECT * FROM orders WHERE status IN (0,1)")
    pending = c.fetchall()

    c.execute("SELECT * FROM orders WHERE status=2")
...
# DECRYPT SECTION
@app.route("/admin/decrypt/<int:order_id>")
def decrypt_order(order_id):
    if "admin" not in session:
        return redirect("/admin/login")
    ...
    c.execute("""
        ...
        FROM orders WHERE id=? AND status=0
    """, (order_id,))
    row = c.fetchone()
...
def download(order_id):
    ...
    c.execute("""
        SELECT filename
        FROM orders WHERE id=? AND status=1
    """, (order_id,))
    row = c.fetchone()
    ...
    filename = row[0]
    output_path = os.path.join(DECRYPT_DIR, filename.replace(".enc", ""))

    c.execute("UPDATE orders SET status=2 WHERE id=?", (order_id,))
    conn.commit()
    conn.close()
